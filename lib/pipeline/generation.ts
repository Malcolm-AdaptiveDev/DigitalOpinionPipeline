...
const PERSONA_TEMPERATURE: Record<PersonaId, number> = {
  ...
  oracle: 0.6, // Clinical + analytical — lowest variance for authority
  rebel: 0.85, // Raw + reactive — highest variance, most "human" feel
  sage: 0.7, // Measured + thoughtful — moderate variance, careful word choice
};
...
const PLATFORM_LENGTH_GUIDANCE: Record<Platform, string> = {
  ...
  tiktok:
    "45–90 seconds spoken (approximately 100–200 words). Hook in first 3 words.",
...
const PILLAR_INSTRUCTIONS: Partial<Record<ContentPillar, string>> = {
  breakthrough:
    "Open with the most surprising specific fact. End with a challenge or question.",
  debunk:
    "Start with the popular claim. Methodically dismantle it with specifics. End with a pointed question.",
  follow_the_money:
    'Name the institution and the incentive explicitly. No vague "they."',
  the_model:
    "State the prediction with explicit confidence percentage. State the key assumption. Give a resolution date.",
  scorecard:
    "Reference a specific prior claim by name or date. Give a verdict. Update the record.",
  mediation:
    "Acknowledge both positions by name. Introduce a third frame that neither side is asking. End with an open question.",
  morning_question:
    "One question only. No context. No explanation. Let it land alone.",
  reluctant_credit:
    "Acknowledge what you were wrong about specifically. Name what changed your mind. Do not minimise — the credibility is in the discomfort.",
  moment_of_softness:
    "No analysis. No critique. Just the thing itself. Two sentences maximum.",
  callout:
    "Name the institution. Name the specific action. Name who benefits. End with an implication, not an accusation.",
  arbiter:
    "Name both sides. Provide the data or philosophical frame neither is using. Do not pick a winner.",
};
...
function buildUserMessage(request: GenerationRequest): string {
  ...
  const pillarInstruction = PILLAR_INSTRUCTIONS[request.pillar] ?? "";
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (isRateLimit && attempt < maxRetries) {
        ...
        console.warn(
          `[Generation] Rate limit on attempt ${attempt}. Retrying in ${Math.round(delay)}ms…`,
        );
...
export function detectCascadeRequests(
  ...
  if (
    ...
    ["breakthrough", "reframe", "future_vision"].includes(
      originalRequest.pillar,
    ) &&
  ...
  const recentNova = networkActivity.find(
    (a) => a.persona === "nova" && a.hours_ago < 2,
  );
  const recentCynic = networkActivity.find(
    (a) => a.persona === "cynic" && a.hours_ago < 2,
  );
  if (
...
export async function generatePost(
  ...
  const { content, inputTokens, outputTokens } = await withTimeout(
    ...
    120000,
    `Anthropic request timed out after 120 seconds for ${request.persona}/${request.pillar}/${request.platform}`,
  );
