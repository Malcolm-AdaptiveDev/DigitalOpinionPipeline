...
function trend(
  ...
  return {
    ...
    memory_score: Math.min(
      1,
      Object.values(memoryCounts).reduce((sum, count) => sum + count, 0) / 10,
    ),
...
function createSeedStore(): MockStore {
  const pendingReq = request(
    "nova",
    "open model releases and developer trust",
    ["open_models", "developer_trust", "ai_infra"],
    "breakthrough",
    "x",
  );
  const approvedReq = request(
    "oracle",
    "semiconductor export policy signals",
    ["semiconductors", "policy", "supply_chain"],
    "signal_report",
    "linkedin",
  );
  const editedReq = request(
    "rebel",
    "creator economy platform fatigue",
    ["creator_economy", "platform_fatigue", "media"],
    "callout",
    "threads",
  );
  const rejectedReq = request(
    "cynic",
    "startup valuation rebound narrative",
    ["startups", "valuation", "market_cycle"],
    "cold_read",
    "x",
  );
  ...
  const reviewQueue: MockReviewRow[] = [
    ...
    },
    {
      ...
      status: "edited",
      final_content:
        "Creator platforms keep selling burnout as a workflow problem. It is cleaner branding than admitting the business model rewards churn.",
  ...
  return {
    ...
    publishedPosts: [
      ...
      },
      {
        ...
        platform: "threads",
        content:
          reviewQueue[2].final_content ?? reviewQueue[2].generation.content,
        published_at: iso(86),
    ...
    latestTrends: [
      trend(
        "mock-trend-open-models",
        "Open model releases shift toward inspectable AI infrastructure",
        ["ai", "open_models", "ai_infra"],
        0.91,
        { ai: 2, open_models: 1, ai_infra: 1 },
        22,
      ),
      trend(
        "mock-trend-semiconductor-policy",
        "New semiconductor export rules reshape advanced packaging demand",
        ["semiconductors", "policy", "supply_chain"],
        0.84,
        { semiconductors: 1, policy: 1, supply_chain: 0 },
        75,
      ),
      trend(
        "mock-hn-agent-tools",
        "Developers push agent tools toward local-first workflows",
        ["tech", "programming", "ai"],
        0.77,
        { tech: 0, programming: 0, ai: 2 },
        130,
      ),
      trend(
        "mock-trend-creator-platforms",
        "Creator platforms test new monetization rules as fatigue rises",
        ["creator_economy", "platform_fatigue", "media"],
        0.69,
        { creator_economy: 1, platform_fatigue: 0, media: 0 },
        230,
      ),
    ],
    rssFeeds: [
...
export function isMockMode(enabled?: boolean): boolean {
  return (
    ...
    process.env.PIPELINE_DATA_SOURCE === "mock"
  );
}
...
export function getMockReviewRows(status: string): MockReviewRow[] {
  return store()
    .reviewQueue.filter((item) => item.status === status)
    .sort(
      (a, b) =>
...
export function handleMockReviewAction(body: {
  ...
  if (!item)
    throw new Error(`Mock review item not found: ${body.reviewItemId}`);
...
export function addMockFeed(
  input: Omit<RssFeed, "id" | "created_at" | "updated_at" | "last_checked_at" | "last_health_status" | "last_health_error" | "consecutive_failures" | "auto_disabled">,
): RssFeed {
...
export function updateMockFeed(id: string, patch: Record<string, unknown>): RssFeed | null {
...
export function runMockHealthCheck(feedId?: string): RssFeedHealthCheckResult[] {
  const feeds = feedId
