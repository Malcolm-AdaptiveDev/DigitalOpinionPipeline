...
function inferRelationalEvents(
  ...
  if (request.crossTagPersona) {
    const isDebate     = ['debunk', 'history_repeating', 'follow_the_money', 'cold_read'].includes(request.pillar)
    const isCredit     = ['reluctant_credit', 'creator_spotlight'].includes(request.pillar)
    const isMediation  = request.pillar === 'mediation'
    const isAlliance   = ['text_and_now', 'moment_of_softness', 'morning_question'].includes(request.pillar)
...
function detectBeliefShift(
  ...
  const shiftSignals: Record<PersonaId, string[]> = {
    nova:   ['i was wrong', 'i underestimated', 'i need to update', 'more complicated than', 'changed my mind'],
    cynic:  ['i was wrong', 'reluctant credit', 'have to admit', 'actually delivered', 'correct on this one'],
    oracle: ['updating my model', 'revising', 'new data changes', 'probability updated', 'was miscalibrated'],
    rebel:  ['have to give credit', 'this one is different', 'surprised me', 'not what i expected'],
    sage:   ['i have been mistaken', 'this reframes', 'i must reconsider', 'the examined assumption'],
  }
...
export async function writePublishedPostToMemory(
  ...
  const finalContent  = reviewItem.final_content ?? reviewItem.generation.content
  ...
  const shiftData  = opts.beliefShift ??
    (autoDetect.isShift
      ? {
  ...
  console.log(`[MemoryWrite] Published post record written for ${persona} / ${platform}`)
}
...
export async function updateEngagementScore(
  ...
  const client = (db as unknown as () => ReturnType<typeof import('./db').db>)()
  const { error } = await client
    .from('episodic_memory')
    .update({ engagement_score: score })
    .eq('id', episodicMemoryId)
  if (error) {
    console.error(`[EngagementUpdate] Failed for ${episodicMemoryId}:`, error.message)
  }
}
