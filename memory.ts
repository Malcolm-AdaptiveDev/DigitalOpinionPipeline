...
function loadTemplate(personaId: PersonaId): string {
  ...
  const path = join(__dirname, '..', 'prompts', `${personaId}_system_prompt_template.md`)
  ...
  const lines = memories
    .map((m, i) => {
      ...
      return (
        ...
        (m.cross_refs?.length ? ` | Cross-persona refs: ${m.cross_refs.length}` : '')
      )
    })
    .join('\n\n')
...
async function buildPositionSummary(
  ...
  const avg = posts.reduce((s, p) => s + (p.engagement_score ?? 0), 0) / posts.length
...
export function buildWorldContext(item: ScoredTrendItem, personaId: PersonaId): WorldContext {
...
export async function assembleMemoryBundle(
  ...
  const relByPersona: Record<PersonaId, RelationalState | null> = {
    nova:   null, cynic:  null, oracle: null, rebel: null, sage: null,
  }
...
export function assemblePrompt(opts: PromptAssemblyOptions): AssembledPrompt {
  ...
  // L3 check
  const missingRelational = (['nova','cynic','oracle','rebel','sage'] as PersonaId[])
    .filter(p => p !== opts.personaId && !opts.bundle.relational[p])
  if (missingRelational.length > 0) {
    warnings.push(`L3: Missing relational state for: ${missingRelational.join(', ')}`)
  }
  ...
  const worldBlock = formatWorldContextBlock(opts.worldContext, opts.networkActivity)
  const beliefBlock = formatBeliefBlock(opts.bundle.beliefEvolution)

  // Build relational replacements for all 4 other personas
  const otherPersonas = (['nova','cynic','oracle','rebel','sage'] as PersonaId[])
  ...
  for (const other of otherPersonas) {
    ...
    relationalReplacements[`${prefix}_TRUST`]     = state?.trust_score.toFixed(2) ?? '0.50'
    relationalReplacements[`${prefix}_TENSION`]   = state?.tension_score.toFixed(2) ?? '0.50'
    relationalReplacements[`${prefix}_SENTIMENT`] = state?.recent_sentiment ?? 'neutral'
    relationalReplacements[`${prefix}_DISPUTES`]  = state?.active_disputes?.join(', ') || 'none'
    relationalReplacements[`${prefix}_SHARED`]    = state?.shared_positions?.join(', ') || 'none'
    relationalReplacements[`${prefix}_RECENT`]    = state?.recent_summary ?? 'No recent interaction.'
  }
