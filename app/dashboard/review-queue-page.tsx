...
async function getData(status: string, mockMode: boolean) {
  if (mockMode) {
    ...
    const trendingCount = getMockTrendingCount();
    const topicTags = Array.from(
      new Set(items.flatMap((row) => topicTagsForRequest(row.request))),
    );
  ...
  const [itemsRes, allRes, trendCountRes] = await Promise.allSettled([
    sb
      .from("review_queue")
      .select("*")
      .eq("status", status)
    ...
    activePersona === "all"
      ...
      : items.filter(
          (r: Record<string, unknown>) =>
            (r.generation as { persona: string })?.persona === activePersona,
        );

  return (
    <PipelineShell
      active={activeTab === "budget" ? "budget" : "review"}
      mockMode={mockMode}
    >
      ...
      >
        <h2
          style={{ fontSize: 18, fontWeight: 500, margin: 0, color: "#ededed" }}
        >
          Review queue
        </h2>
        <Link
      ...
      {mockMode && (
        ...
        >
          <span style={{ fontSize: 13, color: "#9EC8FF" }}>
            Mock server mode: sample review items and webhook actions are
            in-memory.
          </span>
          <Link
            href={routeBase}
            style={{ fontSize: 12, color: "#9EC8FF", textDecoration: "none" }}
          >
            Use live data
          </Link>
        </div>
        ...
        {[
          {
            key: "queue",
            label: "Review queue",
            href: `${routeBase}?status=${activeStatus}&persona=${activePersona}${mockParam}`,
          },
          {
            key: "budget",
            label: "Budget settings",
            href: `/dashboard?tab=budget${mockParam}`,
          },
        ].map((tab) => (
          <Link
            key={tab.key}
        ...
        <>
          ...
          {counts.pending > 0 && activeStatus !== "pending" && (
            ...
            >
              <span style={{ fontSize: 13, color: "#EF9F27" }}>
                {counts.pending} posts still pending review
              </span>
              <Link
                href={`${routeBase}${mockQuery}`}
                style={{
                  fontSize: 12,
                  color: "#EF9F27",
                  textDecoration: "none",
          ...
          >
            {(
              ...
              <Link
                key={s}
                href={`${routeBase}?status=${s}&persona=${activePersona}${mockParam}`}
                ...
                >
                  ...
                  <div
                    style={{
                      fontSize: 26,
                      fontWeight: 500,
                      color: s === activeStatus ? STATUS_COLOR[s] : "#d7dbe4",
                    }}
                  >
                    {counts[s]}
                  </div>
          ...
          >
            {(["all", "nova", "cynic", "oracle", "rebel", "sage"] as const).map(
              (p) => (
                <Link
                  key={p}
                  href={`${routeBase}?status=${activeStatus}&persona=${p}${mockParam}`}
                  style={{ textDecoration: "none" }}
                >
                  <div
                    style={{
          ...
          {filtered.length === 0 ? (
            ...
            >
              ...
              {activePersona !== "all" ? ` for ${activePersona}` : ""}.
            </div>
          ) : (
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <i
                        className={`ti ${PLATFORM_ICON[gen.platform] ?? "ti-world"}`}
                        style={{ fontSize: 13 }}
                        aria-hidden="true"
                      />
                      {gen.platform}
                    ...
                    {gen.warnings?.length > 0 && (
                      ...
                      >
                        <i
                          ...
                          aria-hidden="true"
                        />
                        {gen.warnings.length} warning
                        {gen.warnings.length > 1 ? "s" : ""}
                      </span>
                    )}
                    <span
                      style={{
                  ...
                  {item.request?.topic && (
                    <div
                      style={{ fontSize: 12, color: "#444", marginBottom: 8 }}
                    >
                      <span style={{ color: "#8c96a8" }}>Topic:</span>{" "}
                      <span style={{ color: "#d6d9e0" }}>
                        {item.request.topic}
                      </span>
                    </div>
                  )}
                  ...
                  {topicTags.length > 0 && (
                    ...
                    >
                      ...
                      {topicTags.map((tag) => {
                        const hasPersonaMemory = memoryTagsByPersona.has(
                          `${gen.persona}:${tag}`,
                        );
                        ...
                        return (
                          <span
                            ...
                            title={
                              hasPersonaMemory
                                ? "Episodic memory for this persona"
                                : hasNetworkMemory
                                  ? "Episodic memory elsewhere in the network"
                            ...
                            style={{
                              ...
                              gap: 5,
                              fontSize: 12,
                              color: hasPersonaMemory
                                ? "#7FE0BA"
                                : hasNetworkMemory
                                  ? "#9EC8FF"
                                  : "#c1c7d0",
                              background: hasPersonaMemory
                          ...
                          >
                            {(hasPersonaMemory || hasNetworkMemory) && (
                              <i
                  ...
                  >
                    ...
                    <span>
                      triggered by {item.request?.triggeredBy ?? "unknown"}
                    </span>
