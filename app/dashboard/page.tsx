...
export default async function DashboardPage({
  ...
  return (
    <PipelineShell active={activeTab === 'budget' ? 'budget' : 'review'}>
      ...
      {mockMode && (
        <div style={{
          ...
          <Link href="/dashboard" style={{ fontSize: 12, color: '#9EC8FF', textDecoration: 'none' }}>Use live data</Link>
        </div>
      )}

      <div style={{ display: 'flex', gap: 8, marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        {[
          { key: 'queue', label: 'Review queue', href: `/dashboard?status=${activeStatus}&persona=${activePersona}${mockParam}` },
      ...
      {counts.pending > 0 && activeStatus !== 'pending' && (
        <div style={{
          ...
          <Link href={`/dashboard${mockQuery}`} style={{ fontSize: 12, color: '#EF9F27', textDecoration: 'none' }}>Review now →</Link>
        </div>
      )}

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 10, marginBottom: '1.5rem' }}>
        {(['pending', 'approved', 'edited', 'rejected'] as const).map(s => (
          <Link key={s} href={`/dashboard?status=${s}&persona=${activePersona}${mockParam}`} style={{ textDecoration: 'none' }}>
            <div style={{
              background:   '#171a22',
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        {(['all', 'nova', 'cynic', 'oracle', 'rebel', 'sage'] as const).map(p => (
          <Link key={p} href={`/dashboard?status=${activeStatus}&persona=${p}${mockParam}`} style={{ textDecoration: 'none' }}>
            <div style={{
              display:     'inline-flex',
              alignItems:  'center',
              gap:         6,
