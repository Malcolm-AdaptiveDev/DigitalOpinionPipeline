...
import { ingestTrends, scoreAndRoute } from '@/lib/pipeline/ingestion'
import {
...
async function processTrendForPersona(
  ...
  try {
    ...
    runErrors.push({ stage: 'memory_assembly', message: (err as Error).message, detail: { personaId, topic: item.topic } })
  ...
  try {
    ...
    runErrors.push({ stage: 'generation', message: (err as Error).message, detail: { personaId, topic: item.topic } })
  ...
  try {
    ...
    console.log(`[Pipeline] Queued for review: ${reviewId} (${personaId}/${platform}/${pillar})`)
  } catch (err) {
...
export async function runTrendPipeline(): Promise<void> {
  ...
  const runErrors: Array<{ stage: string; message: string; detail?: unknown; at: string }> = []
  let trendsF = 0, trendsS = 0, generated = 0, queued = 0
  ...
  try {
    ...
    try {
      ...
      runErrors.push({ stage: 'ingestion', message: (err as Error).message, at: new Date().toISOString() })
      console.error('[Pipeline] Ingestion failed — aborting run:', err)
      await completePipelineRun(runId, { trendsF, trendsS, generated, queued }, runErrors)
      return
    }
    ...
    try {
      ...
      runErrors.push({ stage: 'scoring', message: (err as Error).message, at: new Date().toISOString() })
    ...
    const allItems    = [...scoredItems, ...unprocessed.filter(
      u => !scoredItems.some(s => s.id === u.id)
    )]
    ...
    const networkActivity: NetworkActivity[] = recentPosts.map(p => ({
      ...
      hours_ago: Math.round((Date.now() - new Date(p.created_at).getTime()) / 3600000),
    ...
    for (const item of allItems) {
      ...
      for (const personaId of item.assigned_personas) {
        const result = await processTrendForPersona(
          item, personaId, networkActivity, runErrors
        )
        if (result) {
          generated++
          queued++
      ...
      if (primaryResults.length > 0) {
        ...
        const cascades = detectCascadeRequests(latestResult, latestRequest, networkActivity)

        for (const cascade of cascades) {
          // Don't generate cascade if persona was already assigned
          if (item.assigned_personas.includes(cascade.persona)) continue

          console.log(`[Pipeline] Cascade: ${cascade.persona} reacts to ${latestResult.persona}`)

          try {
            const cascadeBundle   = await assembleMemoryBundle(cascade.persona, item.topic)
            ...
            generated++
            queued++
            console.log(`[Pipeline] Cascade queued: ${cascade.persona}/${cascade.platform}`)
            ...
            console.error(`[Pipeline] Cascade failed for ${cascade.persona}:`, err)
          }
        }
      }
...
export async function processApprovedPost(
  ...
  if (item.status !== 'pending') {
    console.warn(`[Publish] Item ${reviewItemId} is already ${item.status} — skipping.`)
    return
  }

  // Update review record
  await updateReviewStatus(reviewItemId, opts?.finalContent ? 'edited' : 'approved', {
  ...
  await writePublishedPostToMemory({
    reviewItem:      { ...item, final_content: opts?.finalContent ?? item.final_content },
    request:         item.request,
    platformPostId:  opts?.platformPostId,
    beliefShift:     opts?.beliefShift,
  })

  console.log(`[Publish] Post ${reviewItemId} processed and written to memory.`)
}
...
export async function runScheduledPosts(): Promise<void> {
  ...
  for (const scheduled of duePosts) {
    try {
      const bundle    = await assembleMemoryBundle(scheduled.persona, scheduled.topic)
      const worldCtx  = {
      ...
      console.error(
        `[Scheduled] Failed for ${scheduled.persona}/${scheduled.pillar}:`, err
      )
    }
  }
}
...
export const approvedPostWebhook = inngest.createFunction(
  ...
  { event: 'post/approved' },
  async ({ event, step }) => {
    const { reviewItemId, finalContent, editorNotes, reviewedBy, platformPostId, beliefShift } =
      event.data as {
        reviewItemId:   string
        finalContent?:  string
        editorNotes?:   string
